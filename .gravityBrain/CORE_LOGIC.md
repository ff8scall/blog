...
6. **WebP 변환 (LCP Optimization) [v6.2]**:
   - `Pillow`를 사용하여 모든 이미지를 WebP 포맷으로 강제 변환합니다.
   - 용량 절감(최대 70%) 및 초기 로딩 속도(LCP) 개선을 통해 사용자 경험과 SEO 지표를 동시에 향상시킵니다.
7. **Path Stability (v3.0)**: 
   - 모든 이미지 생성 및 검색 기준을 프로젝트 루트 `/static`으로 일원화하여 배포 누락을 방지합니다.
