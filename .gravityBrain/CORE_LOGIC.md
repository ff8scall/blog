...
3. **지능형 폴백 (Context-Aware Fallback)**:
   ...
   - [v11.6] **콘텐츠 유실 방지 폴백**: NLM 출력물에서 `KOR_CONTENT`가 누락된 경우 `KOR_SUMMARY`나 `KOR_INSIGHT`를 본문으로 자동 전환함과 동시에, 제목 추출 소스를 요약 필드까지 확장하여 '빈 기사' 발생을 방지합니다. 제목 제한을 80자로 확장하여 정보 전달력을 개선했습니다.
    - [v12.1] **NLM 출력 변종 대응 (Robust Parsing)**: NotebookLM이 필드 구분자(콜론)를 생략하거나 ### 헤더를 필드명으로 사용하는 경우에도 데이터 유실 없이 추출할 수 있도록 정규식을 강화했습니다.
    - [v12.2] **슬러그 유니크성 강화 (Unique Slug)**: 비영어권 제목 정제 시 발생하는 슬러그 충돌을 방지하기 위해, 모든 기사 슬러그 뒤에 고유 ID를 추가하여 파일명 중복 및 기사 유실을 원천 차단했습니다.
...
3. **헤더 강화 (User-Agent)**: 
   ...
   - [v1.5] **Streaming Mode 전환**: Bing/Naver 등 검색 엔진의 부하 최적화 권고에 따라, 다수 URL을 한 번에 보내는 'Batch' 방식에서 각 URL을 즉시 개별 통보하는 'Streaming(GET)' 방식으로 전송 알고리즘을 개편하였습니다. 이를 통해 색인 실시간성을 극대화합니다.
4. **연동 검증**: 
