...
class NewsHarvester:
    ...
    def __init__(self, test_mode=False):
        ...
        self.keyword_pools = {
            "llm_tech": ["Large Language Model architecture", "Transformer MoE efficiency", "Reasoning models", "Context window expansion", "Synthetic data training"],
            "ai_agent": ["Agentic workflows", "Autonomous planning", "Multi-agent orchestration", "Tool-use AI", "Self-evolving agents"],
            "ai_policy": ["AI regulation EU Act", "Big tech antitrust AI", "AI safety guidelines", "Ethics in machine learning"],
            "future_sw": ["AI-driven coding", "Natural language programming", "Generative UI UX", "SaaS AI integration"],
            "semi_hbm": ["HBM4 roadmap", "NVIDIA Blackwell specs", "2nm GAA process", "Custom AI ASIC", "Advanced Packaging CoWoS"],
            "hpc_infra": ["AI supercomputer", "Liquid cooling data center", "Photonics interconnect", "Hyperscale infrastructure"],
            ...
            "startups_vc": ["AI startup funding trends", "Silicon Valley VC dry powder", "Strategic M&A tech sector"],
            "market_trend": ["Semiconductor trade war", "Big tech earnings analysis", "Global tech policy", "Digital sovereignty"],
            "game_tech": ["Unreal Engine 5 Nanite", "Neural rendering", "AI NPC procedural", "Handheld gaming innovation"],
            "spatial_tech": ["Vision Pro spatial computing", "XR industrial use-cases", "MR hardware optics", "Metaverse pivot"]
        }

        self.categories_config = {
            "llm_tech": {"kor_name": "LLM & 모델링", "thenews_cat": "tech", "currents_cat": "science_technology", "newsdata_cat": "technology"},
            "ai_agent": {"kor_name": "자율 에이전트", "thenews_cat": "tech", "currents_cat": "science_technology", "newsdata_cat": "technology"},
            "ai_policy": {"kor_name": "글로벌 규제·정책", "thenews_cat": "tech", "currents_cat": "politics", "newsdata_cat": "politics"},
            "future_sw": {"kor_name": "지능형 소프트웨어", "thenews_cat": "tech", "currents_cat": "science_technology", "newsdata_cat": "technology"},
            "semi_hbm": {"kor_name": "반도체 & 메모리", "thenews_cat": "tech", "currents_cat": "science_technology", "newsdata_cat": "technology"},
            "hpc_infra": {"kor_name": "고성능 컴퓨팅(HPC)", "thenews_cat": "tech", "currents_cat": "science_technology", "newsdata_cat": "technology"},
            ...
            "startups_vc": {"kor_name": "스타트업 & 투자", "thenews_cat": "business", "currents_cat": "economy_business_finance", "newsdata_cat": "business"},
            "market_trend": {"kor_name": "글로벌 경제·동향", "thenews_cat": "business", "currents_cat": "economy_business_finance", "newsdata_cat": "business"},
            "game_tech": {"kor_name": "게임 테크", "thenews_cat": "entertainment", "currents_cat": "arts_culture_entertainment", "newsdata_cat": "entertainment"},
            "spatial_tech": {"kor_name": "공간 컴퓨팅", "thenews_cat": "tech", "currents_cat": "science_technology", "newsdata_cat": "technology"}
        }
    ...
    def _get_dynamic_query(self, cat_key):
        return random.choice(self.keyword_pools.get(cat_key, ["Technology"]))
    ...
    def _categorize_article(self, title, description):
        ...
        if any(k in text for k in hw_keywords):
            ...
            if any(k in text for k in ["server", "data center", "hpc"]): return "hpc_infra"
            return "semi_hbm"
        
        if any(k in text for k in game_keywords): return "game_tech"
        ...
        if any(k in text for k in money_keywords):
            if any(k in text for k in ["funding", "vc ", "investment", "startup"]): return "startups_vc"
            if any(k in text for k in ["revenue", "saas", "pricing"]): return "monetization"
            return "market_trend"
        
        # AI/Software priority
        if "agent" in text or "autonomous" in text: return "ai_agent"
        if "policy" in text or "regulation" in text or "antitrust" in text: return "ai_policy"
        if "llm" in text or "gpt" in text or "transformer" in text or "model" in text: return "llm_tech"
    ...
    def fetch_all(self, limit_per_cat=8, rss_only=False):
        ...
        for cat_key, config in self.categories_config.items():
    ...
    def _fetch_newsapi(self, query, kor_name, limit):
        ...
        url = f"https://newsapi.org/v2/everything?q={query}&sortBy=publishedAt&language=en&pageSize={limit}&apiKey={self.keys['newsapi']}"
        try:
